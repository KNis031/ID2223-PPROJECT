...
### Project Architecture and Apps
The architecture and full project pipeline can be described like this. The full dataset was dowloaded from [Zendo.org](https://zenodo.org/records/3887261) and put on google drive. Using colab, a CNN was trained, validated and tested. The model + utils along with infrastructure for storing images, sounds and predictions were registered with [hopsworks](https://www.hopsworks.ai/). 

A batch inference script were deployed on [modal](https://modal.com/). The script runs every three days, and starts by fetching the model and dictionary of previous predictions from hopsworks. Afterwards, the script GET the 5 most recent sounds on freesound.org, pre-process them and runs them through the model. The dictionary of previous predictions is updated and is pushed togheter with the most recent .ogg files and spectrograms to hopsworks. In this way, the sound files and spectrograms are overwitten every 3 days by a new run, but the JSON of predictions are kept (see next section).
